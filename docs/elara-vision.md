# Elara: A BEAM-Native Fabric for Software Work

> **Status:** Design exploration  
> **Updated:** August 2026  
> **Purpose:** Capture the architectural ideas, product primitives, risks, and
> experiments behind evolving Elara beyond a conventional coding-agent CLI.

## Executive thesis

Elara should not become “Pi, but written in Elixir,” nor should its ambition stop
at running more agents in more containers.
...
Elara could become a larger control and execution fabric:
...
must fit before it reaches Elara’s generic tool-result truncator; content is
...
Local Elara can use an embedded journal. A hosted deployment can use a boring
...
Compare single-agent Elara with fresh delegated research, delegated
...
Prove Elara can be a recoverable work runtime before clustering it.
...
3. Elara-specific durable workflow combinators.
...
## Evolution of the current Elara code

| Current area           | Evolution                                                                                                                   |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `Elara.Session.Core`    | Remains the deterministic reducer. Keep IO, clocks, PIDs, and retries outside it.                                           |
| `Elara.Session`         | Becomes an activated durable-entity shell with journal commit, outbox dispatch, and job reconciliation.                     |
| `Elara.FlightRecorder`  | Becomes causal export, debugger, replay engine, and counterfactual laboratory over canonical facts.                         |
| `Elara.Session.Store`   | Becomes a transcript/content projection behind a storage behavior.                                                          |
| `Elara.Executor.Router` | Evolves from direct invocation into placement of durable job intents while preserving capability and uncertainty semantics. |
| `Elara.Worker.Server`   | Becomes a workspace-cell agent with durable receipts, resumable streams, fencing, and sandbox integration.                  |
| `Elara.Server`          | Becomes an authenticated gateway and entity locator with durable cursors and principal-aware commands.                      |
...
1. **Elara runtime** — trusted BEAM control plane and durable entities.
