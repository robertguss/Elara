defmodule Elara.Plugin do
  ...
  def discover(cwd) when is_binary(cwd) do
    cwd
    |> Path.join(".elara/plugins/*.{ex,exs}")
    |> Path.wildcard()
    |> Enum.sort()
  end
  ...
  @doc false
  @spec run(map(), Ctx.t()) :: Elara.Tool.outcome()
