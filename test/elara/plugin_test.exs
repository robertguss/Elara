defmodule Elara.PluginTest do
  ...
  setup do
    dir = Path.join(System.tmp_dir!(), "elara-plugin-#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
  ...
  defp start_session(opts) do
    {:ok, session} = Elara.start_session(Keyword.merge([persist: false, tools: []], opts))
  ...
  defp await_reload(session, attempts) do
    case Elara.reload_plugins(session) do
  ...
  test "reload changes code without changing the session, state process, state, or history", %{
    ...
    prior_history = Elara.transcript(session)
    assert tool_outcomes(session) == [{:ok, "1 count=1"}]

    write_counter(path, module, "2")

    assert {:ok, [after_reload]} = Elara.reload_plugins(session)
    ...
    assert Elara.transcript(session) == prior_history
    ...
    assert {:ok, "second done"} = Elara.ask(session, "second")
  ...
  test "a failed multi-plugin reload aborts every prepared candidate", %{
    ...
    before_infos = Elara.plugins(session)
    ...
    assert Elara.plugins(session) == before_infos
  ...
  test "state migration activates atomically and a failed migration does not", %{
    ...
    assert {:ok, [%{version: "2", generation: 2}]} = Elara.reload_plugins(session)
    assert {:ok, "second done"} = Elara.ask(session, "second")
    assert tool_outcomes(session) == [{:ok, "1 count=1"}, {:ok, "2 count=2"}]
    ...
    assert [%{version: "2", generation: 2}] = Elara.plugins(session)
  end
  ...
  test "reload stays busy after interrupt until the checked-out plugin call ends", %{
    ...
    assert :ok = Elara.ask_async(session, "start")
    assert_receive {:elara, ^session, {:tool_started, %ToolCall{name: "counter"}}}

    [plugin] = Elara.plugins(session)
    ...
    assert {:error, :busy} = Elara.reload_plugins(session)

    Elara.interrupt(session)
    assert_receive {:elara, ^session, {:turn_ended, :interrupted}}
    write_counter(path, module, "2")

    assert {:error, {:plugin_reload_failed, ^path, :busy}} = Elara.reload_plugins(session)
  ...
  test "tool timeout releases the plugin lease without committing state", %{path: path, dir: dir} do
    ...
    assert {:ok, "timed out"} = Elara.ask(session, "first")
    assert tool_outcomes(session) == [{:error, "timed out"}]

    write_counter(path, module, "2")
    assert {:ok, [%{version: "2", generation: 2}]} = Elara.reload_plugins(session)
    assert {:ok, "after reload"} = Elara.ask(session, "second")
  ...
  test "a queued deadline wins without committing a completed invocation", %{
    ...
    :ok = Elara.subscribe(session)
    assert :ok = Elara.ask_async(session, "first")
    ...
    assert_receive {:elara, ^session, {:turn_ended, {:completed, "timed out"}}}

    assert {:ok, "after timeout"} = Elara.ask(session, "second")
  ...
  test "two sessions keep independent code revisions and state", %{path: path, dir: dir} do
    ...
    assert {:ok, "first"} = Elara.ask(first, "first")
    assert {:ok, "first"} = Elara.ask(second, "first")

    [first_v1] = Elara.plugins(first)
    [second_v1] = Elara.plugins(second)
    assert first_v1.module == second_v1.module
    assert first_v1.pid != second_v1.pid

    write_counter(path, module, "2")
    assert {:ok, [first_v2]} = Elara.reload_plugins(first)
    assert first_v2.module != second_v1.module

    assert {:ok, "second"} = Elara.ask(first, "second")
    assert {:ok, "second"} = Elara.ask(second, "second")
  ...
  test "a tool-name collision leaves the old registry generation active", %{
    ...
    assert Elara.plugins(session) == [before]
    assert {:ok, "done"} = Elara.ask(session, "use the old tool")
    assert tool_outcomes(session) == [{:ok, "1 count=1"}]
  end
