defmodule Harness.Attach do
  ...
  def main(argv) do
    ...
    case {args, invalid} do
      ...
      _ -> fail("usage: mix harness.attach SESSION|new [--observe] [--port PORT]")
    end
  end
  ...
  defp handle_server_message(
    ...
    case Protocol.decode_event(encoded) do
      {:ok, event} ->
        IO.write(Harness.CLI.render(event))
  ...
  defp cursor_path(session) do
    Path.join([System.user_home!(), ".harness", "attach", "#{session}.json"])
  end

  defp env_port do
    case Integer.parse(System.get_env("HARNESS_SERVER_PORT", Integer.to_string(@default_port))) do
