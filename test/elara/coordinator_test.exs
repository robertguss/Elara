defmodule Elara.CoordinatorTest do
  use ExUnit.Case, async: false

  alias Elara.Coordinator
  ...
  defp parent_session do
    ...
    assert {:ok, "parent answer"} = Elara.ask(parent, "parent question")
    parent
  end
  ...
  test "three isolated coding candidates survive one child death and a judge selects a winner" do
    ...
    parent_history = Elara.transcript(parent)
    test_pid = self()

    factory = fn spec -> {BlockingProvider, {test_pid, spec.id}} end

    {:ok, coordinator} =
      Elara.start_coordinator(parent,
    ...
    assert Elara.transcript(parent) == parent_history
  ...
  test "coding isolation options cannot be overridden and startup failures clean worktrees" do
    ...
    assert Elara.cwd(child.session_id) == child.worktree
    assert Elara.status(child.session_id).recording_path == nil
    ...
    {:ok, interrupted} =
      Elara.start_coordinator(parent, provider_factory: blocking_factory)
    ...
    {:ok, hostile_id} = Elara.start_coordinator(parent, provider_factory: safe_factory)
